...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    //Homework  - Printing the top 10 popular super heroes with their correlation
    val top10MostPopular = flipped.sortByKey().top(10)
    val top10MostPopularNames = top10MostPopular.map(x => (namesRdd.lookup(x._2)(0), x._1) )
    println("Top 10 popular heroes are : \n")
    for( res <- top10MostPopularNames){
      println(s"${res._1} is a popular superhero with ${res._2} co-appearances.")
    }
  }
